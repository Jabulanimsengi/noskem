...
CREATE OR REPLACE FUNCTION "public"."handle_item_purchase"("p_buyer_id" "uuid", "p_item_id" bigint) RETURNS bigint
    ...
    AS $$
DECLARE
    v_purchase_fee INT; v_current_balance INT; v_item RECORD; v_new_order_id BIGINT;
BEGIN
...
BEGIN
    ...
    FOR abandoned_order IN
        ...
        WHERE o.status = 'pending_payment' AND o.created_at < now() - interval '15 minutes'
    LOOP
...
CREATE POLICY "Comprehensive Item View Policy" ON "public"."items" FOR SELECT USING ((("status" = ANY (ARRAY['available'::"public"."item_status", 'pending_payment'::"public"."item_status", 'sold'::"public"."item_status"])) OR ("auth"."uid"() = "seller_id") OR (EXISTS ( SELECT 1
   FROM "public"."orders"
  WHERE (("orders"."item_id" = "items"."id") AND ("orders"."buyer_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."orders"
  WHERE (("orders"."item_id" = "items"."id") AND (("orders"."agent_id" = "auth"."uid"()) OR (("public"."get_user_role"("auth"."uid"()) = 'agent'::"text") AND ("orders"."status" = 'payment_authorized'::"public"."order_status"))))))));
...
CREATE POLICY "Sellers can manage their own items" ON "public"."items" USING (("auth"."uid"() = "seller_id")) WITH CHECK (("auth"."uid"() = "seller_id"));
...
CREATE POLICY "Users can insert and manage their own items" ON "public"."items" USING (("auth"."uid"() = "seller_id"));
