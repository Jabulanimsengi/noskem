...
CREATE OR REPLACE FUNCTION "public"."cancel_order_and_refund"("p_order_id" bigint, "p_buyer_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_order RECORD;
    v_purchase_fee INT;
BEGIN
    -- 1. Fetch the order and lock it to prevent race conditions
    SELECT * INTO v_order FROM public.orders WHERE id = p_order_id FOR UPDATE;

    -- 2. Security and status checks
    ...
    -- 3. Get the purchase fee from constants
    SELECT "PURCHASE_FEE" INTO v_purchase_fee FROM public.constants LIMIT 1;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'System configuration error: Purchase fee not found.';
    END IF;

    -- 4. Update the item status back to 'available'
    UPDATE public.items
    ...
    -- 6. Refund the purchase fee credits to the user's profile
    UPDATE public.profiles
    SET credit_balance = credit_balance + v_purchase_fee
    WHERE id = p_buyer_id;

    -- 7. Log the credit refund transaction for the user's history
    INSERT INTO public.credit_transactions(profile_id, amount, description, transaction_type)
    VALUES (p_buyer_id, v_purchase_fee, 'Refund for cancelled order #' || p_order_id, 'refund');

    RAISE NOTICE 'Order #% cancelled and fee refunded successfully.', p_order_id;
END;
$$;


ALTER FUNCTION "public"."cancel_order_and_refund"("p_order_id" bigint, "p_buyer_id" "uuid") OWNER TO "postgres";
...
BEGIN
    ...
    SELECT count(*) INTO order_count FROM public.orders WHERE status = 'completed' OR status = 'funds_paid_out';
    SELECT COALESCE(sum(final_amount), 0) INTO total_revenue FROM public.orders WHERE status = 'completed' OR status = 'funds_paid_out';
...
CREATE OR REPLACE FUNCTION "public"."process_verified_payment"("p_order_id" bigint, "p_paystack_ref" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_order RECORD;
...
BEGIN
    -- Step 1: Idempotency Check. If the order already has a Paystack ref, it's been processed.
    -- This is the most important step to prevent duplicate payments.
    IF EXISTS (SELECT 1 FROM public.orders WHERE id = p_order_id AND paystack_ref IS NOT NULL) THEN
        RAISE NOTICE 'Order #% has already been processed. Skipping.', p_order_id;
        RETURN;
    END IF;

    -- Step 2: Get commission rate from constants
    SELECT "COMMISSION_RATE" INTO v_commission_rate FROM public.constants LIMIT 1;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'System configuration error: Commission rate not found.';
    END IF;

    -- Step 3: Update the order status and lock the row
    UPDATE public.orders
    SET status = 'payment_authorized', paystack_ref = p_paystack_ref
    WHERE id = p_order_id
    RETURNING * INTO v_order;
    ...
    IF v_order IS NULL THEN
        RAISE EXCEPTION 'Order #% not found during payment processing.', p_order_id;
    END IF;

    -- Step 4: Update the corresponding item's status to 'sold'
    UPDATE public.items
    SET status = 'sold'
    WHERE id = v_order.item_id
    RETURNING * INTO v_item;

    -- Step 5: Create all necessary financial transaction records
    v_commission_amount := v_order.final_amount * v_commission_rate;

    -- Transaction for the Buyer's purchase
    INSERT INTO public.financial_transactions (order_id, user_id, type, status, amount, description)
    VALUES (p_order_id, v_order.buyer_id, 'sale', 'completed', -v_order.final_amount, 'Purchase of item: ' || v_item.title);

    -- Transactions for the Seller's sale and commission
    INSERT INTO public.financial_transactions (order_id, user_id, type, status, amount, description)
    VALUES
      (p_order_id, v_order.seller_id, 'sale', 'pending', v_order.final_amount, 'Sale of item: ' || v_item.title),
      (p_order_id, v_order.seller_id, 'commission', 'completed', -v_commission_amount, 'Commission for: ' || v_item.title);

    -- Step 6: Create notifications for the seller and agents
    -- Notify Seller
    ...
    -- Notify all Agents of the new task
    FOR agent_profile IN SELECT id FROM public.profiles WHERE role = 'agent' LOOP
        PERFORM create_single_notification(
            agent_profile.id,
            'New task available: Item "' || v_item.title || '" needs inspection.',
...
ALTER FUNCTION "public"."process_verified_payment"("p_order_id" bigint, "p_paystack_ref" "text") OWNER TO "postgres";
...
BEGIN
    ...
    LOOP
        -- Refund the credit transaction for the purchase fee
        ...
        -- Log the financial transaction for the refund
        ...
        -- Revert the item and order status
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "verification_status" "public"."verification_status" DEFAULT 'not_verified'::"public"."verification_status",
...
CREATE POLICY "Admins have full access" ON "public"."profiles" TO "authenticated" USING ("public"."is_admin"("auth"."uid"()));
