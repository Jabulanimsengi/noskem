...
import ConfirmCollectionButton from './ConfirmCollectionButton';
import InspectionModalTrigger from './InspectionModalTrigger';

export const dynamic = 'force-dynamic';

// The AgentTask type needs to match the possible statuses
...
export default async function AgentDashboardPage() {
    ...
    // FIX: Filter active tasks based on the correct statuses from your documentation
    const activeTasks = myAgentTasks.filter(task => 
        ['awaiting_assessment', 'pending_admin_approval', 'awaiting_collection', 'in_warehouse'].includes(task.status)
    );
                    ...
                    <table className="min-w-full">
                        ...
                        <tbody className="divide-y divide-gray-200 text-sm">
                            {availableTasks.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="p-6 text-center text-text-secondary">There are no new tasks available.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                            ...
                            {activeTasks.length > 0 ? (
                                activeTasks.map(task => (
                                    <tr key={task.id}>
                                        ...
                                        <td className="p-3 text-right">
                                            {/* FIX: Renders the correct button based on the actual task status from your docs */}
                                            {task.status === 'awaiting_assessment' && (
                                                <InspectionModalTrigger orderId={task.id} />
                                            )}
                                            {task.status === 'awaiting_collection' && (
                                                <ConfirmCollectionButton orderId={task.id} />
                                            )}
                                            {(task.status === 'pending_admin_approval' || task.status === 'in_warehouse') && (
                                                <span className="px-3 py-1.5 text-xs text-gray-500">Awaiting Next Step</span>
                                            )}
                                        </td>
                                    </tr>
                                ))
                                ...
                                <tr>
                                    <td colSpan={4} className="p-6 text-center text-text-secondary">You have no active tasks.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
