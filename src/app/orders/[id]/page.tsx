...
// Ensure the Item type includes 'buy_now_price'
type ItemWithPrice = Item & {
    buy_now_price: number | null;
};

type OrderWithItem = Order & {
    items: ItemWithPrice | null;
    delivery_fee_paid?: number; // Make sure this field is in your Order type
};
...
export default async function OrderPage({ params }: OrderPageProps) {
    ...
    // Calculate the base item price for the summary
    const itemPrice = order.final_amount - (order.delivery_fee_paid || 0);
            ...
            <div className="bg-surface rounded-lg shadow-lg p-8">
                ...
                <div className="bg-background rounded-lg p-4 flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        {/* The main price is now the total, which is clearer */}
                        <p className="text-2xl font-bold text-brand mt-1">
                            Total: R{order.final_amount.toFixed(2)}
                        </p>
                    </div>
                </div>

                {/* --- Updated Price Breakdown Section --- */}
                <div className="space-y-3 text-md mb-8 border-t border-b border-gray-200 py-4">
                    <div className="flex justify-between">
                        <span className="text-text-secondary">Item Price:</span>
                        <span className="font-medium text-text-primary">R{itemPrice.toFixed(2)}</span>
                    </div>
                    {order.delivery_fee_paid && order.delivery_fee_paid > 0 && (
                        <div className="flex justify-between">
                            <span className="text-text-secondary">Collection & Delivery:</span>
                            <span className="font-medium text-text-primary">R{order.delivery_fee_paid.toFixed(2)}</span>
                        </div>
                    )}
                    <div className="flex justify-between font-bold text-lg pt-2 border-t border-dashed">
                        ...
                        <span className="text-brand">R{order.final_amount.toFixed(2)}</span>
                    </div>
                </div>
                {/* --- End of Section --- */}
